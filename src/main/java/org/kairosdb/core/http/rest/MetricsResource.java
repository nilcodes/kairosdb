...
public class MetricsResource implements KairosMetricReporter
{
	...
	public Response getVersion()
	{
		...
		ResponseBuilder responseBuilder =
				Response.status(Response.Status.OK).entity("{\"version\": \"" + versionString + "\"}\n");
	...
	@Produces(MediaType.APPLICATION_JSON + "; charset=UTF-8")
	@Path("/his/query")
	public Response HistoryQuery(@QueryParam("tags") String tags,
					@QueryParam("start") long start,
					@QueryParam("end") long end,
					@QueryParam("iv") int timeInterval) throws InvalidServerTypeException {

		try {
			checkArgument(tags != null && tags.length() > 0, "查询指标名称不能为空");
			checkArgument(start >= 0, "查询开始时间必须大于0");
			checkArgument(end > 0 && end > start, "结束时间必须大于开始时间");
			checkArgument(timeInterval >=1000, "间隔步长不能少于1秒，即1000毫秒。");

			checkArgument((end-start)/timeInterval <= 500000,"数据集过大，请缩小查询区间或查询步长");


			final String[] tagNames = tags.split(",");

			Query query = new Query();
			query.setStartAbsolute(start);
			query.setEndAbsolute(end);
			//query.setCacheTime(10);

			for (String tagName : tagNames) {
				final QueryMetric metric = new QueryMetric(start, 0, tagName);
				final AvgAggregator avgAggregator;

				avgAggregator = new AvgAggregator(new DoubleDataPointFactoryImpl());

				avgAggregator.setSampling(new Sampling(timeInterval, TimeUnit.MILLISECONDS));
				//avgAggregator.setAlignSampling(true);
				//avgAggregator.setAlignStartTime(true);
				avgAggregator.setStartTime(start);
				avgAggregator.setAlignStartTime(true);
				metric.addAggregator(avgAggregator);
				query.addQueryMetric(metric);
			}

			final Query mainQuery = m_queryPreProcessor.preProcess(query);
			List<QueryMetric> queries = mainQuery.getQueryMetrics();


			Map<String,InterpolationDataPoint> dataPoint = new HashMap<>();

			queries.forEach(queryItem -> {
				try (DatastoreQuery dq = datastore.createQuery(queryItem)) {
					List<DataPointGroup> results = dq.execute();

					Map<Long,Double> pointData = new TreeMap<>();
					for (DataPointGroup dg : results) {
						dg.forEachRemaining(curr -> {
							pointData.put(curr.getTimestamp(), curr.isDouble()
									? curr.getDoubleValue() : curr.getLongValue());
						});
						dataPoint.put(dg.getName(), new InterpolationDataPoint(pointData));
					}

				} catch (DatastoreException e) {
					logger.error("并行查询异常",e.getMessage());
				}
			});

			checkArgument(dataPoint!=null,"datapoint null value");

			List<InterpolationResult> HistoryResult = new ArrayList<>();

			List<Long> timeRanges = new ArrayList<>();
			for(Long tagTimestamp = start;tagTimestamp<=end;tagTimestamp+=timeInterval){
				timeRanges.add(tagTimestamp);
			}

			dataPoint.forEach((key,val)->{
				checkArgument(key!=null,"key为空");
				checkArgument(val!=null,"val为空");

				InterpolationResult interpolationResult = new InterpolationResult(key,timeRanges.size());
				timeRanges.parallelStream().forEach(tagTimestamp->{
					final Double valueByTimestamp = val.getValueByTimestamp(tagTimestamp);
					interpolationResult.Add(new InterpolationData(tagTimestamp,valueByTimestamp));
				});

				HistoryResult.add(interpolationResult);
			});

			ResponseBuilder responseBuilder = Response.status(Response.Status.OK)
					.entity(HistoryResult);

			setHeaders(responseBuilder);
			return responseBuilder.build();

		} catch (KairosDBException e) {
			logger.error("Kairos DB failed.", e);
